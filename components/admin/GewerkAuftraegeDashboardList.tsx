      ...
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          ...
          <p className={`mt-1 max-w-xl text-xs leading-snug ${sub}`}>
            Auftrag antippen: Termin unter „Termin festlegen“ setzen oder ändern. Übersicht hier in der Liste mit Datum
            auf der Karte.
          </p>
        </div>
        <button
            ...
            return (
              <li key={auftrag.id}>
                <article
                  role="button"
                  tabIndex={0}
                  onClick={() => onSelectAuftrag(auftrag)}
                  onKeyDown={(e) => e.key === "Enter" && onSelectAuftrag(auftrag)}
