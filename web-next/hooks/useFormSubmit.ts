...
export function useFormSubmit() {
  ...
  const submitForm = async <T extends object>(
    ...
    try {
      ...
      // Prüfe auf Placeholder-Konfiguration oder fehlende ENV-Vars
      if (error) {
        // Prüfe, ob es ein Konfigurationsproblem ist (Placeholder-URL oder ungültiger API-Key)
        const isConfigError = 
        ...
        if (isConfigError) {
          console.warn('Supabase-Konfiguration fehlt oder ist ungültig.');
          toast({
            variant: "destructive",
            title: "Konfiguration fehlt",
            description: "Die Formular-Konfiguration ist noch nicht vollständig. Bitte versuchen Sie es später erneut oder kontaktieren Sie uns direkt.",
          });
